...
struct CreatePlannerView: View {
    @State var x:Int = 0
    @Binding var isShowed: Bool
    @State  var showAddTrashoutModal : Bool = false
    @State var isEditTrashout: Bool = false
            ...
            Form {
                Section(header: Text("Routine info")){
                    ...
                    Picker("Routine length", selection: $routineLength) {
                        ...
                        print(routineLength + selectedweek)
                        if (routineLength=="1"){
                            weeks = ["1"]
...
//                        .padding(.bottom, 50.0)
                        ...
                        ForEach(items) { item in
                                ...
                                ZStack{
                                   TrashoutCard(item: item)
                                    NavigationLink(destination: EditTrashoutView(newItem: item, isEditTrashout: $isEditTrashout, routineLength: $routineLength),isActive: $isEditTrashout){
                                    }
            ...
            .navigationBarTitle("Edit Planner")
